...
export const StatusBadge = ({ status }: { status?: ReadStatus }) => {
	...
	if (status === "reading") {
		return (
			<span className="flex-none inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-[#219190]/15 text-[#57C785] border border-[#219190]/30">
				<span className="relative flex h-1.5 w-1.5" aria-hidden>
					<span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#57C785] opacity-75" />
					<span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[#57C785]" />
				</span>
				Currently reading
			</span>
		);
...
export const TakeLine = ({ take }: { take?: string }) => {
	if (!take) return null;
	return (
		<p className="text-sm italic text-[#57C785]/90 leading-relaxed mt-3 md:mt-0 md:max-h-0 md:opacity-0 md:overflow-hidden md:group-hover:mt-3 md:group-hover:max-h-24 md:group-hover:opacity-100 transition-all duration-200 ease-out">
			<span aria-hidden>» </span>
			{take}
		</p>
	);
