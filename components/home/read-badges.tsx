...
export const StatusBadge = ({ status }: { status?: ReadStatus }) => {
	...
	if (status === "reading") {
		return (
			<span className="flex-none inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-[#3B82F6]/15 text-[#93C5FD] border border-[#3B82F6]/30">
...
export const TakeLine = ({ take }: { take?: string }) => {
	if (!take) return null;
	return (
		<p className="text-sm italic text-[#93C5FD]/90 leading-relaxed mt-3 md:mt-0 md:max-h-0 md:opacity-0 md:overflow-hidden md:group-hover:mt-3 md:group-hover:max-h-24 md:group-hover:opacity-100 transition-all duration-200 ease-out">
			<span aria-hidden>» </span>
			{take}
		</p>
	);
