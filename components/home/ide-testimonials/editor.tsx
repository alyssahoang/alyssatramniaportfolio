...
const buildLines = (file: ITestimonialFile, color: string): ILineModel[] => [
	...
	...file.quoteLines.map((line, i) => ({
		highlight: i === 0,
		segs: [
			{ t: "> ", cls: "text-[#57C785]/70", noSelect: true },
...
const Caret = () => (
	<span
		className="ide-caret inline-block w-[8px] h-[15px] align-text-bottom bg-[#57C785] ml-px"
		aria-hidden="true"
	/>
);
		...
		>
			...
			<div className="absolute top-10 right-4 md:top-12 md:right-8 z-10 pointer-events-none">
				<Image
					...
					width={112}
					height={112}
					className="w-16 h-16 md:w-28 md:h-28 rounded-full object-cover border-4 border-[#219190]/40 shadow-lg shadow-[#219190]/20"
					loading="lazy"
				/>
			</div>
			...
			>
				{rendered.map(({ line, visible, hasCaret }, i) => (
					<div
						key={i}
						className={`flex ${line.highlight ? "bg-[#219190]/10" : ""}`}
					>
