...
const Assistant = ({
	...
	return (
		<div className="flex flex-col bg-gray-900/40 min-w-0 lg:h-full">
			...
			>
				<VscSparkle className="text-[#57C785]" aria-hidden="true" />
				<span className="font-mono text-xs font-semibold tracking-widest bg-gradient-to-r from-[#219190] to-[#57C785] bg-clip-text text-transparent">
					ALYSSABOT
				</span>
					...
					{messages.map((msg, i) =>
						msg.role === "user" ? (
							<div key={i} className="flex justify-end">
								<p className="max-w-[85%] rounded-xl rounded-br-sm bg-gradient-to-br from-[#219190]/40 to-[#219190]/20 border border-[#219190]/40 px-3 py-2 text-xs text-gray-100">
									{msg.text}
								</p>
							</div>
						) : (
							<div key={i} className="flex flex-col items-start gap-2">
								<p className="max-w-[90%] rounded-xl rounded-bl-sm bg-gray-800/70 border border-gray-700/60 px-3 py-2 text-xs text-gray-200 leading-relaxed">
									...
									{!msg.done && (
										<span
											className="ide-caret inline-block w-[7px] h-[13px] align-text-bottom bg-[#57C785] ml-0.5"
											aria-hidden="true"
										/>
									)}
								</p>
										...
										{msg.authors.map((author) => {
											...
											return (
												<button
													...
													className="flex items-center gap-1.5 pl-1 pr-2.5 py-1 rounded-full bg-gray-800/80 border border-gray-700/60 text-[11px] text-gray-300 hover:border-[#219190]/60 hover:text-white transition-all duration-[10ms]"
												>
													<Image
														src={file.avatar}
							...
							<button
								...
								className={`text-[11px] px-2.5 py-1 rounded-full border transition-all duration-[10ms] ${
									asked.has(theme.label)
										...
										: "bg-[#219190]/15 border-[#219190]/25 text-[#57C785] hover:bg-[#219190]/30 hover:border-[#219190]/50"
								}`}
							>
