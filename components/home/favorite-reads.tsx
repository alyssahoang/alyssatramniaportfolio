...
const ReadFavicon = ({ read }: { read: IFavoriteRead }) => {
	...
	if (failed) {
		return (
			<div className="flex-none w-11 h-11 rounded-xl bg-[#219190]/15 border border-[#219190]/20 flex items-center justify-center text-[#57C785] font-semibold text-lg">
				{initial}
			</div>
		);
	}
	...
	<div className="p-6 flex flex-col h-full">
		<div className="flex items-center gap-4 mb-4">
			...
			<div className="min-w-0">
				<h4 className="font-semibold text-white group-hover:text-[#57C785] transition-colors duration-[10ms] leading-snug text-base truncate">
					{read.title}
				</h4>
		...
		>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{visible.map((read, index) => (
					<a
						...
						rel="noreferrer"
						data-read-card
						className="group block rounded-2xl overflow-hidden bg-gray-900/80 backdrop-blur-sm border border-gray-800/50 transition-all duration-[10ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:border-[#219190]/40 hover:shadow-[0_20px_40px_-12px_rgba(33,145,144,0).15)] hover:-translate-y-2"
						onClick={() => {
			...
			{hidden > 0 && (
				<button
					...
					className="self-center mt-8 text-sm font-medium px-5 py-2 rounded-full border border-[#219190]/30 text-[#57C785] hover:border-[#219190]/60 hover:bg-[#219190]/10 transition-all duration-[10ms]"
				>
