...
const CertificateSection = ({ isDesktop }: IDesktop) => {
	...
	const renderCertificate = (cert: typeof CERTIFICATES[number]): React.ReactNode => (
		<div key={cert.name} className="cert-card group h-full">
			<div className="card-shine h-full flex flex-col rounded-2xl bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 shadow-xl overflow-hidden hover:border-[#219190]/30 hover:bg-gray-800/70 hover:shadow-[0_0_30px_-5px_rgba(33,145,144,0).2)] hover:-translate-y-2 transition-all duration-[10ms]">
				...
				<div className="mt-auto px-5 py-4 border-t border-gray-700/50 text-left">
					...
					<span className="inline-block mt-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#219190]/10 text-[#57C785] border border-[#219190]/20">
						{cert.issuer}
					</span>
				</div>
			</div>
